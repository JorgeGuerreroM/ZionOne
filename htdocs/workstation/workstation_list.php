	...
	if ($mode == 'kanban') {
		...
		if (!empty($arrayfields['wug.fk_usergroup']['checked'])) {
			...
			foreach ($object->usergroups as $id_group) {
				$g = new UserGroup($db);
				$g->fetch($id_group);
				$toprint[] = '<li class="select2-search-choice-ZionOne noborderoncategories'.($cssforli ? ' '.$cssforli : '').'" style="background: #bbb">' . $g->getNomUrl(1, '', 0, 'categtextwhite') . '</li>';
			}
		...
		if (!empty($arrayfields['wr.fk_resource']['checked'])) {
			...
			foreach ($object->resources as $id_resource) {
				...
				$r->fetch($id_resource);
				$toprint[] = '<li class="select2-search-choice-ZionOne noborderoncategories'.($cssforli ? ' '.$cssforli : '').'" style="background: #bbb">' . $r->getNomUrl(1, '', '', 0, 'categtextwhite') . '</li>';
			}
