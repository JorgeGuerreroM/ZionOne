...
function htmlPrintOnlineHeader($mysoc, $langs, $showlogo = 1, $alttext = '', $subimageconst = '', $altlogo1 = '', $altlogo2 = '')
{
	...
	if ($showlogo) {
		if (!empty($mysoc->logo_small) && is_readable($conf->mycompany->dir_output . '/logos/thumbs/' . $logosmall)) {
			...
			$urllogopublic = $dolibarr_main_url_root . '/theme/logo.png';
		}
	}
	...
	if ($urllogopublic || $alttext) {
		...
		if (!getDolGlobalString('MAIN_HIDE_POWERED_BY')) {
			print '<div class="poweredbypublicpayment opacitymedium right hideonsmartphone"><a class="poweredbyhref" href="https://www.ZionOne.org?utm_medium=website&utm_source=poweredby" target="ZionOne" rel="noopener">' . $langs->trans("PoweredBy") . '<br><img class="poweredbyimg" src="' . DOL_URL_ROOT . '/theme/logo.png" width="80px"></a></div>';
		}
		print '</div>';
	}
