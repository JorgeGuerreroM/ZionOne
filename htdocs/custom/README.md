# ZionOne ERP & CRM custom directory for external modules
...
WARNING !!!
Check also that the /custom directory is active by adding into ZionOne `conf/conf.php` file the following
two lines, so ZionOne will also scan /custom directory to find external external modules:
