...
if ($action == 'unseteditinline') {	// Test on permission not required here
	zionone_del_const($db, 'WEBSITE_EDITINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.urlencode(GETPOST('website')).'&pageid='.GETPOSTINT('pageid'));
	exit;
}
...
if ($action == 'unsetshowsubcontainers') {	// Test on permission not required here
	zionone_del_const($db, 'WEBSITE_SUBCONTAINERSINLINE');
	header("Location: ".$_SERVER["PHP_SELF"].'?website='.urlencode(GETPOST('website')).'&pageid='.GETPOSTINT('pageid'));
	exit;
}
...
if ($action == 'addcontainer' && $usercanedit) {
			...
			if (!$error) {
				...
				foreach ($regs[0] as $key => $val) {
					...
					if (!empty($tmpgeturl['curl_error_no'])) {
						...
						// We try to convert the CSS we got by adding a prefix .bodywebsite with lessc to avoid conflict with CSS of ZionOne.
				...
				// Normalize links href to ZionOne internal naming
	...
	if (!$error) {
		...
		if (!dol_is_file($filerobot)) {
			$robotcontent = "# Robot file. Generated with ZionOne\nUser-agent: *\nAllow: /public/\nDisallow: /administrator/";
		...
		if (!dol_is_file($filereadme)) {
			$readmecontent = "Website generated by ZionOne ERP CRM";
...
if (!GETPOST('hide_websitemenu')) {
	...
	if ($websitekey && $websitekey != '-1' && (!in_array($action, array('editcss', 'editmenu', 'importsite', 'file_manager', 'replacesite', 'replacesiteconfirm', 'editsecurity'))) && (!in_array($mode, array('replacesite'))) && !$file_manager) {
		...
		if ($action == 'editsource' || $action == 'editcontent' || GETPOST('editsource', 'alpha') || GETPOST('editcontent', 'alpha')) {
			$url = 'https://wiki.ZionOne.org/index.php/Module_Website';
...
if ((empty($action) || $action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone') && !in_array($mode, array('replacesite'))) {
	if ($pageid > 0 && $atleastonepage) {
		...
		// Output page under the ZionOne top menu
		...
		*/
		...
		// Clean style that may affect global style of ZionOne
