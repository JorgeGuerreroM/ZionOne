	...
	if ($ok) {
		...
		if (count($filelist) == 0) {
			...
			foreach ($filelist as $file) {
				...
				// Scan if there is migration scripts that depends of ZionOne version
				...
				if (count($modulesfile)) {
					...
					foreach ($modulesfile as $modulefilelong => $modulefileshort) {
						...
						// Run sql script
						$okmodule = run_sql($modulefilelong, 0, 0, 1); // Note: Result of migration of external module should not decide if we continue migration of ZionOne or not.
