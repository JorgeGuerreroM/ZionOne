...
if (!$error && $db->connected && $action == "set") {	// Test on permission not required here
	...
	if (!$error) {
		...
		// We test access with ZionOne database user (not admin)
		if (!$error) {
...
function write_conf_file($conffile)
{
	...
	if ($fp) {
		...
		fwrite($fp, '<?php'."\n");
		fwrite($fp, '//'."\n");
		fwrite($fp, '// File generated by ZionOne installer '.DOL_VERSION.' on '.dol_print_date(dol_now(), '')."\n");
		fwrite($fp, '//'."\n");
		...
		fwrite($fp, '$dolibarr_main_authentication=\'ZionOne\';');
		fwrite($fp, "\n\n");
